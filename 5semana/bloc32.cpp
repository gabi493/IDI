...
//------------------bloc3.2------------------
double extremTerra = sqrt((.75*.75) + (.75*.75));
double tamEsferaCont = sqrt((extremTerra*extremTerra) + (1.*1.));	//se calcula a mano
...
void mouseMoved(int x, int y) {
	...
	if (changeState) {
		newx2 += float(x-oldx)*1.5/float(w);
		oldx = x;
	//-------------------hacer que no se salga del cuadrado----------------------------
		if (newx2 > 0) newx2 = 0;		//el modelo esta abajo a la derecha
		if (newx2 < -2*lado + ancho_escalado) newx2 = -2*lado + ancho_escalado;
	//---------------------------------------------------------------------------------	

		newz2 += float(y-oldy)*1.5/float(h);
		oldy = y;
	//-------------------hacer que no se salga del cuadrado----------------------------
		if (newz2 > 0) newz2 = 0;		//el modelo esta abajo a la derecha
		if (newz2 < -2*lado + profundo_escalado) newz2 = -2*lado + profundo_escalado;		
	//---------------------------------------------------------------------------------	
	}
}
